...
<%
    ...
    boolean hasReminderQuestions = true;
    if (user2 != null) {
        hasReminderQuestions =  user2.hasReminderQuery();
        ArrayList<String> secQuestions = new ArrayList<>();
        for (int i = 2; i <= 5; i++) {
            if ((user2.getExpandoBridge().getAttribute("security-question-"+i) != null && user2.getExpandoBridge().getAttribute("security-question-"+i).toString().isEmpty()) ||
                    (user2.getExpandoBridge().getAttribute("security-question-answer-"+i) != null && user2.getExpandoBridge().getAttribute("security-question-answer-"+i).toString().isEmpty())){
                hasReminderQuestions = false;
            }
        }
    }
                ...
                <c:if test="<%= Validator.isNotNull(user2.getReminderQueryQuestion()) && Validator.isNotNull(user2.getReminderQueryAnswer()) %>">
                    ...
                    <% for (int i = 1; i <= 5; i++) {%>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <aui:input label="<%= HtmlUtil.escape(LanguageUtil.get(request, user2.getExpandoBridge().getAttribute("security-question-"+i).toString())) %>" name="answer${i}" type="text" />
                            </c:otherwise>
                        </c:choose>
                    <% } %>
                </c:if>
                <c:choose>
                    <c:when test="<%= PropsValues.USERS_REMINDER_QUERIES_REQUIRED && !hasReminderQuestions %>">
                        <div class="alert alert-info">
